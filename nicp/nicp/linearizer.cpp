...
void Linearizer::update() {
  ...
  for (int32_t i = 0; i < numThreads; ++i) {
    _Htt[i] = Eigen::Matrix4f::Zero();
    _Htr[i] = Eigen::Matrix4f::Zero();
    _Hrr[i] = Eigen::Matrix4f::Zero();

    _bt[i] = Eigen::Vector4f::Zero();
    _br[i] = Eigen::Vector4f::Zero();
  }
...
#pragma omp parallel
  {
    ...
    Eigen::Matrix4f Htt = Matrix4f::Zero();
    Eigen::Matrix4f Htr = Matrix4f::Zero();
    Eigen::Matrix4f Hrr = Matrix4f::Zero();
    Eigen::Vector4f bt = Vector4f::Zero();
    Eigen::Vector4f br = Vector4f::Zero();
