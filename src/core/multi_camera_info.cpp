...
namespace nicp {
  ...
  void MultiCameraInfo::serialize(nicp::ObjectData& data, nicp::IdContext& context) {
    BaseCameraInfo::serialize(data,context);
    nicp::ArrayData* camera_info_array = new nicp::ArrayData;
    for (std::vector<BaseCameraInfo*>::iterator it = _camera_infos.begin(); it!=_camera_infos.end(); it++){
      BaseCameraInfo* cam=*it;
      camera_info_array->add(new nicp::PointerData(cam));
    }
    data.setField("cameraInfos", camera_info_array);
  }

  void MultiCameraInfo::deserialize(nicp::ObjectData& data, nicp::IdContext& context){
    BaseCameraInfo::deserialize(data,context);
    nicp::ArrayData& camera_info_array=data.getField("cameraInfos")->getArray();
    ...
    for(size_t i = 0; i<camera_info_array.size(); i++){
      nicp::ValueData& v = camera_info_array[i];
