...
namespace nicp {
  ...
  void Solver::errorAndJacobian(Eigen::Vector3f&  pointError, 
    ...
    // std::cerr << "[DEBUG] reference normal " << referenceNormal.transpose() << std::endl;
    // std::cerr << "[DEBUG] current   normal " << tn.transpose() << std::endl;
    // std::cerr << "[DEBUG] n error " << normalError.transpose() << std::endl;
    //if(isNan(pointError)) { exit(-1); } 
    // std::cerr << "[DEBUG] n error " << normalError.transpose() << std::endl;
    //if(isNan(normalError)) { exit(-1); } 
