...
namespace nicp {
  ...
  template <class SampleType>
    ...
    _SigmaPoint(const SampleType& sample, double wi, double wp):
      _sample(sample), _wi(wi), _wp(wp){}
    _SigmaPoint(): _wi(0), _wp(0) {}
      SampleType _sample;
      double _wi;
      double _wp;
    };
  ...
  template <class SampleType, class CovarianceType>
    bool sampleUnscented(std::vector<_SigmaPoint <SampleType>, Eigen::aligned_allocator<_SigmaPoint <SampleType> > >& sigmaPoints, const SampleType& mean, const CovarianceType& covariance){
    ...
    sigmaPoints[0] = _SigmaPoint<SampleType>(mean, 
					    lambda/(dim + lambda), 
    ...
    for (int i=0; i<dim; i++) {
      ...
      sigmaPoints[k++]=_SigmaPoint<SampleType>(mean + s, wi, wi);
      sigmaPoints[k++]=_SigmaPoint<SampleType>(mean - s, wi, wi);
    }
    return true;
  }
  ...
  template <class SampleType, class CovarianceType>
    void reconstructGaussian(SampleType& mean, CovarianceType& covariance, 
			     const std::vector<_SigmaPoint<SampleType>, Eigen::aligned_allocator<_SigmaPoint <SampleType> > >& sigmaPoints){
