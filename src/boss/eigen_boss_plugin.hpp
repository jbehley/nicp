...
inline void toBOSS(nicp::ObjectData& data, const std::string name) const {
  nicp::ObjectData* matrixData = new nicp::ObjectData;
  if (SizeAtCompileTime == Eigen::Dynamic) {
    matrixData->setInt("rows", rows());
    matrixData->setInt("cols", cols());
  }
  nicp::ArrayData * adata =  new nicp::ArrayData();
  adata->reserve(rows()*cols());
  for (int r=0; r<rows(); r++)
    for (int c=0; c<cols(); c++)
      adata->push_back(new nicp::NumberData((float)this->operator()(r,c)));
  matrixData->setField("values",adata);
  data.setField(name, matrixData);
}

inline void fromBOSS(nicp::ObjectData& data, const std::string name){
  nicp::ObjectData& matrixData = data.getField(name)->getObject();
  if (SizeAtCompileTime == Eigen::Dynamic) {
  ...
  nicp::ArrayData& adata = matrixData.getField("values")->getArray();
